...
Этот форк добавляет **транспорт-агностичный режим работы MCP-сервера**: помимо исходного локального HTTP-сервера, расширение умеет работать как WS-клиент к внешнему оркестратору `v8-client-session-manager`. Включены два новых общих модуля (`Мсп_ТранспортСессионКлиент`, `Мсп_СерверОтветы`), dual-mode startup `/C mcpMode=ws|http|auto`, dual-language hooks (EN+RU БСП) в `ManagedApplicationModule` и обновлённый bundle компоненты `AddIn.WebTransport` 0.7.0.

Полное описание изменений, сценарий использования, распределение ответственности между Rust- и BSL-сторонами (установка WS-соединения, пинги, `session.register`) — см. **[FORK_CHANGES.md](FORK_CHANGES.md)**.
